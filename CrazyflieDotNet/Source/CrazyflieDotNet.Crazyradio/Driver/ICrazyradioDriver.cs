...
namespace CrazyflieDotNet.Crazyradio.Driver
{
	...
	public interface ICrazyradioDriver
	{
		...
		/// <summary>
		///     The number of times to retry sending data after an acknowledgement packagte (ACK) is not received when
		///     communicating with Crazyflie quadcopters.
		/// </summary>
		MessageAckRetryCount? AckRetryCount { get; set; }

		/// <summary>
		///     The delay before trying to resend data when an acknowledgement packet (ACK) is not received when communicating with
		///     Crazyflie quadcopters.
		///     If set to UseAckPacketMethod, AckPayloadLength is used for delay time, where time is length in seconds of the
		///     AckPayloadLength.
		/// </summary>
		...
		///     If AckRetryDelay is set to UseAckPacketMethod, AckPayloadLength is used for retry delay time, where time is length
		///     in seconds of the AckPayloadLength.
		/// </summary>
